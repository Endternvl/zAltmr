...
const client = require('../index'); //defines the index
  ...
  client.on("guildMemberAdd", async member => {
    ...
    let ch =
    await client.data.get(`welmsg_${member.guild.id}`) || client.db.get(`welmsg_${member.guild.id}`) || "Hello and welcome to {server}, {member}";
    const messs = ch
      .split(`{member}`)
