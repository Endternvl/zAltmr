...
module.exports = async ({ message, pages } = {}) => {
    let i = 0;
    let embed = await message.reply({ embeds: [pages[i]] });

    ["⏪", "◀️", "⏹", "▶️", "⏩"].map(x => embed.react(x));
    ...
    function next() {
        embed.edit({ embeds: [pages[i]] });
    };

    let collector = embed.createReactionCollector({ filter })
